...
Player::Player(QGraphicsItem *parent)
{
    ...
    connect(timer1,SIGNAL(timeout()),this,SLOT(movimiento_salto()));
    timer1->start(100);
...
void Player::spawn()
{
    if(game->score->getMuertos()>=0 && game->score->getMuertos()<35 ){
    Enemy * enemy = new Enemy();
    scene()->addItem(enemy);
    }
    else if(game->score->getMuertos()>=35 && game->score->getMuertos()<200){
        Enemy * enemy = new Enemy();
        scene()->addItem(enemy);
